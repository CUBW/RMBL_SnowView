...
DEST_FILEPATH = 'data/640x800/'

import rasterio
import numpy as np

def pad_cut_geotiff(input_path, output_path, target_width=640, target_height=800):
    """
    Pad a GeoTIFF image with np.nan values, or crop it, to achieve the target width and height.
    ...
    Parameters:
        ...
        target_height (int): The desired height of the padded image. Default is 800.
    """
    with rasterio.open(input_path) as src:
        ...
        if(original_height > target_height):
            ...
            crop_start = (original_height - target_height) // 2
            crop_end = crop_start + target_height

            # Crop the image
            padded_data = data[:, crop_start:crop_end, :]
...
for input_path in tqdm(glob.glob(os.path.join(SOURCE_FILEPATH, '*.tif')), unit='image'):
    ...
    pad_cut_geotiff(input_path, output_path)
